...
> **N.B.** We never run `install.packages()` in our quarto doc (we run it once only in our R console) as it would re-install the package everytime we render our report
...
- the **geom** layer (how you want the plot draw, e.g. points, lines, etc.)
...
> Q. Make a histogram of lifeExp facetd by continent (try using `fill=continent` or `col=continent`)
